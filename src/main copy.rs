...
fn main() {
	let client =
		redis::Client::open("redis://127.0.0.1:32853/").expect("Cannot open redis connection");
	let mut con = client
		.get_connection()
	...
	thread::spawn(move || {
		...
		loop {
			let tenant: String = r.recv().unwrap();
			let conf_str: String = con.get(tenant.clone()).unwrap();
			let mut conf: Config = serde_json::from_str(&conf_str).unwrap();

			let count_key = conf.get_count_key();
			let total: usize = con.get(count_key.clone()).unwrap();
			if total > 0 {
				conf.total = total;
